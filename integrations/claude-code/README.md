...
| `recallCrossRun` | `auto` | `MUBIT_CC_RECALL_CROSS_RUN` | Whether recall asks the server for lessons learned in **other runs**, alongside this run's own memory. Real value, billed on a lane that cannot be bounded by a run id — so it costs what the whole instance costs and gets slower on its own as one fills up, no matter what this plugin does. On a hosted instance it measured ~1.7s of a ~2.0s recall: five times the rest of the query put together, for the one extra item it returned. `auto` spends it where there is room and declines it where there is not, reading the budget the caller already has — a hook answering inside the host's 3s prompt timeout declines it, `recallAsync`'s detached refresh takes it — so the expensive half of recall follows the same trade `recallAsync` already makes, without a second thing to tune. `on` pays for it everywhere, which is only coherent with `recallAsync` on or with `MUBIT_CC_RECALL_BUDGET_MS` and `MUBIT_CC_TIMEOUT_MS` raised together to fund it; `off` never asks. It costs **0 LLM calls** either way — it is one field on a request already being sent. |
