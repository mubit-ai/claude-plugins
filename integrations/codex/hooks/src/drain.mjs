...
// @ts-check
/**
 * `hooks/src/drain.mjs` — the Codex entry point for the detached `drain` pass.
 *
 * It is absent from `hooks.json` on purpose — `capture.mjs` and `stage-prompt.mjs` spawn it
 * directly, detached — but it is still exec'd by path, so it needs the same shim and the same
 * runtime floor as a registered hook.
 *
...
await import('../../../claude-code/hooks/src/drain.mjs');
