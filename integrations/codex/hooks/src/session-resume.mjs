...
// @ts-check
/**
 * `hooks/src/session-resume.mjs` — the Codex entry point for the detached resume briefing.
 *
 * It is absent from `hooks.json` on purpose — `session-start.mjs` spawns it directly, detached
 * — but it is still exec'd by path, so it needs the same shim and the same runtime floor as a
 * registered hook. Without this file the shared `session-start` body would call
 * `spawnDetached('session-resume')`, `resolveScript` would find nothing next to it or under
 * `${CLAUDE_PLUGIN_ROOT}/hooks/`, and the whole feature would be a silent no-op on one of the
 * two hosts this plugin ships to.
 *
...
await import('../../../claude-code/hooks/src/session-resume.mjs');
